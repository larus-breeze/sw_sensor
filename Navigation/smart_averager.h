...
template <class value_t> class smart_averager
{
public:
  ...
  void record_input ( value_t current_value, float heading)
  {
    unsigned index = (unsigned) (heading * ONE_DIV_2PI * N_SECTORS);
    ASSERT(index < N_SECTORS);

    sector_averages[index] =
          sector_averages[index] * 0.75f
        + current_value * 0.25f;

    // prepare new output here @ 10Hz for performance reasons
    value_t average={0};
...
private:
  void fill_recordings_with_old_average (void)
  {
