...
void CAN_output ( const output_data_t &x)
{
  ...
  if( false && (x.circle_mode == CIRCLING)) // todo patch search for correct instant wind algorithm while circling
    {
      // use average wind here
      ...
      p.data_h[1] = (int16_t)(round(VSQRTF( sqr(x.wind_average.e[EAST])+ sqr(x.wind_average.e[NORTH])) * 3.6f));
    }
  else
    {
//      ASSERT( isnormal(x.wind.e[EAST]) &&  isnormal(x.wind.e[NORTH]));
